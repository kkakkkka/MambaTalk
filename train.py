...
def main_worker(rank, world_size, args):
    ...
    if len(args.resume.replace(" ", "")) > 0:
        other_tools.load_checkpoints(trainer.model, args.resume, args.g_name)        
        logger.info(f"Training from resume {args.resume} ...")
    else:
    ...
    for epoch in range(args.epochs+1):
        ...
        if epoch >= args.test_start_epoch and (epoch) % args.test_period == 0 and epoch != 0:
            if rank == 0:
                other_tools.save_checkpoints(os.path.join(trainer.checkpoint_path, f"last_{epoch}.bin"), trainer.model, opt=None, epoch=None, lrs=None)
                trainer.test(epoch)
